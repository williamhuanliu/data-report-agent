...
export function ThemeToggle() {
  ...
  useEffect(() => {
    ...
    if (!context) {
      ...
      setLocalResolved(stored === 'dark' ? 'dark' : stored === 'light' ? 'light' : isDark ? 'dark' : 'light');
    }
  }, [context]);

  const theme = context?.theme ?? localTheme;
  const resolvedTheme = context?.resolvedTheme ?? localResolved;
  const setTheme = context?.setTheme ?? ((t: Theme) => {
    setLocalTheme(t);
  ...
  return (
    <button
      type="button"
      onClick={cycleTheme}
      className="flex items-center gap-2 h-9 px-3 text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:text-[var(--foreground)] rounded-lg hover:bg-[var(--surface-elevated)] transition-colors"
      title={getLabel()}
