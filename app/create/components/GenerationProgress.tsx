...
export function GenerationProgress({ progress, currentSection, outline }: GenerationProgressProps) {
  return (
    <div className="max-w-md mx-auto text-center py-16">
      {/* Animated icon */}
      <div className="w-20 h-20 mx-auto mb-8 rounded-2xl bg-gradient-to-br from-[var(--color-gradient-start)] to-[var(--color-gradient-end)] flex items-center justify-center relative">
        <svg
      ...
      <div className="w-full h-2 bg-[var(--border)] rounded-full overflow-hidden mb-4">
        <div
          className="h-full bg-gradient-to-r from-[var(--color-gradient-start)] to-[var(--color-gradient-end)] transition-all duration-500 ease-out"
      ...
      {outline && (
        <div className="mt-8 text-left">
          <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-3">报告结构</p>
          <div className="space-y-2">
            {outline.sections
              .filter((s) => s.enabled)
              .map((section, index) => {
                const sectionProgress = (index / outline.sections.filter((s) => s.enabled).length) * 100;
