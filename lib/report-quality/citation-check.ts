...
function matchesValueInWan(mentionValueInWan: number, validSet: Set<number>): boolean {
...
export function checkCitations(
  ...
  const validValuesInWan = citationList.length > 0 ? buildValidValuesInWan(citationList) : new Set<number>();
  ...
  const checkText = (source: string, text: string) => {
    ...
    for (const m of mentions) {
      ...
      if (valueInWan != null && Number.isFinite(valueInWan) && validValuesInWan.size > 0) {
