...
export function DataChatDialog({
  ...
  const buildConversationContext = (): string => {
    ...
    const lines = messages.map((m) => `${m.role === 'user' ? '用户' : '助手'}: ${m.content}`).slice(-10);
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
      title="与数据对话"
      description={fileName ? `基于「${fileName}」提问，AI 将根据数据回答` : '根据上传的数据提问'}
      size="lg"
    >
      {/* Gamma 风格：左侧简要信息 + 右侧对话区，或单栏对话 */}
      <div className="flex flex-1 min-h-0">
        {/* 左侧：数据摘要 - 小屏可折叠或隐藏 */}
        <aside className="hidden sm:flex flex-col w-56 flex-shrink-0 border-r border-[var(--border)] bg-zinc-50/50 dark:bg-zinc-900/50">
          <div className="p-4 text-xs text-zinc-500 dark:text-zinc-400 space-y-1">
            <p className="font-medium text-[var(--foreground)]">数据概览</p>
            <p>{data.rows.length} 行 × {data.headers.length} 列</p>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          >
              ...
              >
                <div
                  className={`
                    ...
                    ${msg.role === 'user'
                      ? 'bg-gradient-to-r from-[var(--color-gradient-start)] to-[var(--color-gradient-end)] text-white rounded-br-md'
            ...
            {error && (
              <div className="rounded-[var(--radius-md)] px-4 py-2 bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400 text-sm">
                {error}
              </div>
            )}
          </div>
          ...
          <div className="flex-shrink-0 border-t border-[var(--border)] p-4 sm:p-6">
            <div className="flex gap-2">
              <textarea
                ...
                rows={1}
                className="flex-1 min-h-[44px] max-h-32 px-4 py-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] text-[var(--foreground)] placeholder:text-zinc-400 resize-none focus:outline-none focus:ring-2 focus:ring-[var(--border-focus)] focus:border-transparent"
                disabled={loading}
              />
              <Button
                variant="primary"
          ...
          {/* 底部：生成报告 */}
          <div className="flex-shrink-0 border-t border-[var(--border)] px-4 sm:px-6 py-4 flex justify-end">
            <Button
              variant="primary"
              size="lg"
